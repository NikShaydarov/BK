...
//Устанавливаются куки для логина и пароля сгенерированного пользователя с помощью setcookie().
      ...
      if(!$errors){
        ...
        try {
          ...
          $stmt -> execute();
//Общий смысл этого кода - он выполняет вставку информации о пользователе в таблицу 'userlogin', которая используется для хранения данных, необходимых для аутентификации пользователя, таких как логин и хэшированный пароль.
          $id=$db->lastInsertId();
